...
def test_ordered_filter_and_merge_have_explicitly_different_results() -> None:
    ...
    assert merge_then_filter.annotations.intervals == (
        BehaviorInterval("groom", 0.0, 0.9, 0.8),
    )
...
def test_split_uses_physical_cut_points_and_maximum_duration() -> None:
    ...
    assert [
        (interval.start_s, interval.stop_s) for interval in result.annotations.intervals
    ] == [(0.0, 1.0), (1.0, 2.0), (2.0, 4.0), (4.0, 4.5), (4.5, 5.0)]
    assert result.ledger[0].action == "split"
    assert all(
        output.source_interval_ids == ("input:000000",)
        for output in result.ledger[0].outputs
    )
...
def test_contextualize_requires_matching_clock_and_session() -> None:
    ...
    with pytest.raises(ValueError, match="different clock_id"):
        apply_interval_policy(
            target,
            IntervalPolicy(
                (ContextualizeIntervals("context", context_source="epochs"),)
            ),
...
def test_operation_specs_reject_ambiguous_values() -> None:
    ...
    with pytest.raises(ValueError, match="between zero and one"):
        ContextualizeIntervals(
            "context", context_source="epochs", minimum_overlap_fraction=1.1
        )
