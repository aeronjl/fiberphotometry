...
Internal engineering checklist. A checked box means an implementation task was
completed and tested — **not** that the method has been scientifically validated,
independently reproduced, or released. Nothing in this repository has been
released; the package is pre-alpha.

For what the package can and cannot currently do, read the
[capability matrix](../docs/methods/capability-matrix.md) instead. For what has
actually been checked against data, read [`../research/`](../research/) and the
frozen protocols in [`../benchmarks/`](../benchmarks/).
...
- [x] `ndx-fiber-photometry` write path with typed acquisition metadata, real
      `isinstance` reads, and `pynwb`/NWB Inspector validation tests
- [ ] Project-configuration route for declaring acquisition metadata to the CLI
