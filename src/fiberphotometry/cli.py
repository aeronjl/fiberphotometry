...
from fiberphotometry.io.acquisition import detect_acquisition_format
...
def _command_qc(source: LoadedSource, args: argparse.Namespace) -> int:
    ...
    payload = {
        "artifact_type": "fiberphotometry_recording_qc",
        "schema_version": "1",
...
def _command_dff(source: LoadedSource, args: argparse.Namespace) -> int:
    ...
    payload = {
        "artifact_type": "fiberphotometry_dff",
        "schema_version": "1",
        "source": _source_json(source),
        "variable": variable,
...
def _command_align(source: LoadedSource, args: argparse.Namespace) -> int:
    ...
    payload = {
        "artifact_type": "fiberphotometry_event_alignment",
        "schema_version": "1",
        "source": _source_json(source),
        "variable": variable,
...
def _command_transients(source: LoadedSource, args: argparse.Namespace) -> int:
    ...
    payload = {
        "artifact_type": "fiberphotometry_transients",
        "schema_version": "1",
        "source": _source_json(source),
        "variable": variable,
        "units": _variable_units(variable),
...
def _load_neurophotometrics_source(
    ...
    raise CommandError(
        ...
        "declare the ROI columns and wavelengths in a project TOML and use "
        "'fiberphotometry run'",
    )
...
def _load_doric_source(
    ...
    if not signals:
        raise CommandError(
            ...
            "declare the dataset paths in a project TOML and use 'fiberphotometry run'",
        )
    channels = tuple(
        DoricChannel(
            group.rsplit("/", 1)[-1],
...
def _tdt_reader() -> Any:
    try:
        ...
        raise CommandError(
            ...
            "install it with: pip install 'fiberphotometry[tdt]'",
        ) from error
    return read_block
...
def _load_tdt_source(
    ...
    if not signals:
        raise CommandError(
            ...
            "declare the store mapping in a project TOML and use 'fiberphotometry run'",
        )
...
def _load_nwb_source(
    ...
    try:
        ...
        raise CommandError(
            ...
            "install them with: pip install 'fiberphotometry[nwb]'",
        ) from error
...
def _preferred_nwb_series(
    ...
    for name in sorted(acquired):
        if series_provenance(nwbfile, name).get("source_variable") == "signal":
            return name
    for name in sorted(acquired):
        comments = str(getattr(series[name], "comments", ""))
        if "fiberphotometry-core-nwb" in comments and '"source_variable": "signal"' in (
            comments
        ):
            return name
...
def _write_dff_nwb(
    ...
    nwbfile = NWBFile(
        session_description=(
            ...
            "the fiberphotometry command line"
        ),
        identifier=(
            f"fiberphotometry-{subject_id}-{session_id}-"
    ...
    nwbfile.add_scratch(
        ...
        name="fiberphotometry_operations",
...
def _provenance(recording: xr.Dataset) -> list[Any]:
    return cast(
        list[Any],
        json.loads(str(recording.attrs.get("fiberphotometry_operations", "[]"))),
    )
