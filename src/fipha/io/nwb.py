...
``fipha_series_channels`` and ``fipha_series_attributes``,
...
CHANNEL_TABLE_NAME = "fipha_series_channels"
ATTRIBUTE_TABLE_NAME = "fipha_series_attributes"
...
def from_nwb_series(
    ...
    *,
    ...
    channel_names = _channel_names(
        nwbfile, series_name, extension_metadata, data.shape[1]
    )
    ...
    recording = make_recording(
        ...
        subject=_identity(subject, "subject", nwbfile, stored_attrs),
        session=_identity(session, "session", nwbfile, stored_attrs),
        attrs=attrs,
    )
...
def _series_description(
    ...
    if acquisition_metadata is None:
        return (
            f"Fiber photometry {variable} exported by fipha; no optical "
            "acquisition metadata was supplied, so no FiberPhotometryTable region "
            "is linked"
        )
    return (
        f"Fiber photometry {variable} exported by fipha with declared "
...
def _record_series_channels(
    ...
    table = _ensure_scratch_table(
        ...
        (
            ...
            ("channel_name", "Channel label used by the fipha package"),
        ),
    )
