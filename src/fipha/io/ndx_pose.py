...
def _trajectory_from_series(
    ...
    *,
    ...
    comments = _fipha_comments(getattr(series, "comments", ""))
    if comments:
...
def _fipha_comments(comments: Any) -> dict[str, Any]:
    try:
        value = json.loads(str(comments))
    except (json.JSONDecodeError, TypeError):
        return {}
    if not isinstance(value, dict) or value.get("schema") != ("fipha-ndx-pose-v1"):
        return {}
    return value
