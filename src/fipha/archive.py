...
from fipha.publication import ATTESTATION_NAME, SIGNATURE_NAME
...
class ArchiveMetadata:
    ...
    language: str = "en"
    artifact_type: Literal["fipha_archive_metadata"] = "fipha_archive_metadata"
...
def load_archive_metadata(path: str | Path) -> ArchiveMetadata:
    ...
    if value.get("artifact_type") != "fipha_archive_metadata":
...
def create_archive_package(
    ...
    *,
    ...
    manifest = (
        json.dumps(
            {
                "artifact_type": "fipha_archive_manifest",
                "files": inventory,
...
def _validate_archive_manifest(value: Any) -> None:
    ...
    if (
        value.get("artifact_type") != "fipha_archive_manifest"
