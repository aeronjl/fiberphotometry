"""Deterministic, auditable policies for externally discovered behavior intervals."""
...
class FilterIntervals:
    ...
    def __post_init__(self) -> None:
        object.__setattr__(
            self, "operation_id", _nonempty(self.operation_id, "operation_id")
        )
        object.__setattr__(
            self, "include_labels", _labels(self.include_labels, "include_labels")
        )
        object.__setattr__(
            self, "exclude_labels", _labels(self.exclude_labels, "exclude_labels")
        )
...
class SplitIntervals:
    ...
    def __post_init__(self) -> None:
        ...
        if self.maximum_duration_s is None and not cuts:
            raise ValueError(
                "split operation requires maximum_duration_s or cut_points_s"
            )
...
class ContextualizeIntervals:
    ...
    def __post_init__(self) -> None:
        ...
        object.__setattr__(
            self, "context_source", _nonempty(self.context_source, "context_source")
        )
        object.__setattr__(self, "labels", _labels(self.labels, "labels"))
        object.__setattr__(
            self, "context_labels", _labels(self.context_labels, "context_labels")
        )
...
class ResolveIntervalOverlaps:
    ...
    def __post_init__(self) -> None:
        ...
        object.__setattr__(
            self, "label_priority", _labels(self.label_priority, "label_priority")
        )
    ...
    for operation in policy.operations:
        if isinstance(operation, FilterIntervals):
            ...
            _validate_context_compatibility(
                annotations, context, operation.context_source
            )
...
def _apply_filter(
    ...
    for item in tracked:
        ...
        if operation.include_labels and interval.label not in operation.include_labels:
            ...
            operation.minimum_duration_s is not None
            and duration < operation.minimum_duration_s
        ):
            reason = "duration below minimum"
        elif (
            operation.maximum_duration_s is not None
            and duration > operation.maximum_duration_s
        ):
            reason = "duration above maximum"
        elif operation.minimum_confidence is not None and (
            interval.confidence is None
            or interval.confidence < operation.minimum_confidence
        ):
            reason = "confidence missing or below minimum"
        if reason is None:
            kept.append(item)
            ledger.append(
                _entry(operation, "kept", "all filter rules passed", (item,), (item,))
            )
        else:
...
def _apply_merge(
    ...
    selected_labels = (
        ...
        if operation.labels
        else {item.interval.label for item in tracked}
    )
    output = [item for item in tracked if item.interval.label not in selected_labels]
    ledger = [
        _entry(operation, "kept", "label outside merge scope", (item,), (item,))
        for item in output
    ]
    output_index = 0
    for label in sorted(selected_labels):
        ...
        for item in items:
            if (
                not groups
                or item.interval.start_s
                - max(member.interval.stop_s for member in groups[-1])
        ...
        for group in groups:
            if len(group) == 1:
                ...
                output.append(item)
                ledger.append(
                    _entry(operation, "kept", "no eligible neighbour", (item,), (item,))
                )
                continue
            ...
            merged = _TrackedInterval(
                ...
                tuple(
                    dict.fromkeys(
                        source for item in group for source in item.source_interval_ids
                    )
                ),
            )
            output_index += 1
...
def _apply_split(
    ...
    for item in tracked:
        ...
        if operation.labels and interval.label not in operation.labels:
            output.append(item)
            ledger.append(
                _entry(operation, "kept", "label outside split scope", (item,), (item,))
            )
            continue
        cuts = [
            value
            for value in operation.cut_points_s
            if interval.start_s < value < interval.stop_s
        ]
        ...
        ledger.append(
            _entry(operation, "split", "declared split boundary", (item,), pieces)
        )
    return output, ledger
...
def _apply_context(
    ...
    for item in tracked:
        ...
        if operation.labels and interval.label not in operation.labels:
            ...
            ledger.append(
                _entry(
                    operation, "kept", "label outside context scope", (item,), (item,)
                )
            )
            continue
        matches = []
...
def _apply_overlaps(
    ...
    for item in ordered:
        ...
        if pieces == [(item.interval.start_s, item.interval.stop_s)]:
            ...
            ledger.append(
                _entry(
                    operation, "kept", "no higher-priority overlap", (item,), (item,)
                )
            )
            continue
        replacements = tuple(
...
def _validate_context_compatibility(
    ...
    if (target.subject, target.session) != (context.subject, context.session):
        raise ValueError(
            f"context source {context_source!r} has a different subject/session"
        )
...
def _result_payload(
    ...
    payload: dict[str, object] = {
        ...
        "policy": {
            ...
            "operations": [
                _operation_payload(operation) for operation in result.policy.operations
            ],
        },
        "ledger": [
            {
...
def _annotations_payload(annotations: BehaviorAnnotations) -> dict[str, object]:
    return {
        ...
        "session": annotations.session,
        "point_events": {
            label: list(times) for label, times in annotations.point_events.items()
        },
        "intervals": [
            {
                "label": interval.label,
