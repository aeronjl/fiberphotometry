...
- `from_nwb_series` accepts core `TimeSeries` and `FiberPhotometryResponseSeries`
  objects, the latter identified by `isinstance` against the imported extension
  class rather than by class name.
- Extension channel-table fields retained when present: location, excitation and
  emission wavelength, indicator, optical fiber, excitation source and
  photodetector.
- `add_recording_to_nwb` always writes a `FiberPhotometryResponseSeries`, and adds
  a `FiberPhotometryTable` region only when acquisition metadata is declared.
