...
| Animal-level scalar/vector inference | **Supported/experimental** | Auditable animal-estimate ledger, paired and independent two-level contrasts, t intervals, animal bootstrap, standardized effects, support, and leave-one-animal-out influence; typed materializers now cover peri-event curves, spontaneous transients, state-band power, and multi-signal association while preserving domain denominators |
...
| Longitudinal learning trajectories | **Interoperable / experimental** | Validated neural-summary handoff to [Behavio](https://github.com/aeronjl/behavio); one frozen public cross-package benchmark exists, broader replication still required |
| Across-session photometry comparability | **Experimental** | Typed outcome-blind identity, preprocessing, coverage, baseline, reference-coupling and sampling diagnostics with warning/refusal states bound into the Behavio handoff |
| Single-signal autocorrelation, PSD and spectrograms | **Experimental** | Gap-separated autocorrelation pairs and window-weighted Welch PSD; complete-window spectrograms retain edge and missing-data evidence; exact-axis PSD and autocorrelation curves now support animal-level pointwise and simultaneous inference |
...
| Coherence and phase analysis | **Experimental** | Window-weighted joint cross-spectra, state conditioning, explicit phase convention and band summaries; complete coherence curves now support exact-axis animal-level pointwise and simultaneous inference; multitaper and population phase uncertainty remain gaps |
...
| Behavior/pose/state discovery | **External** | Not a fipha capability. Pose/ethogram adapters (DeepLabCut, SLEAP, Keypoint-MoSeq, BORIS, movement), matched-pulse clock synchronization, and interval/bout policies live in [Behavio](https://aeronjl.github.io/behavio/observed-behaviour/), installed via `fipha[behavior]` |
| Standardized `ndx-pose` NWB exchange | **Experimental** | Native loss-aware `ndx-pose` 0.3 inspection plus 2D/3D round trip against Behavio's `PoseTrajectory`; unsupported device/video links are declared omissions; a real camera-to-photometry clock fixture remains a gap ([contract](../ndx-pose-interoperability.md)) |
