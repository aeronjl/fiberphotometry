...
- `median_mad` uses the median candidate score plus a declared robust-sigma
  multiplier of `1.4826 * MAD`, and refuses a source whose score MAD is zero
  instead of falling back to a floor;
- `empirical_quantile` uses a declared quantile of the source score distribution.

Both estimators consume every scored local maximum inside a finite run, including
negative scores and maxima that the detector's own native threshold would reject.
Gating or sign-truncating the population first would bias the median up and the
MAD down, so the frozen value is a robust null over ungated scores rather than a
statistic of the detections it later gates.
...
- `GuppyTransientDetectorSpec` implements GuPPY's chunked two-threshold
  ...
  threshold before the second MAD threshold and local-maximum detection. Both
  multipliers scale the **unscaled** MAD, matching the published GuPPy
  implementation, so `detection_mad=3` is `3 * 0.6745 = 2.02` Gaussian standard
  deviations. `TransientDetectionSpec.threshold` and
  ...
  there. Do not compare the multipliers as if they shared units. Because GuPPy
  detects inside fixed chunks, a local maximum that falls on a chunk edge cannot
  be evaluated; it is recorded as a `chunk_boundary` exclusion rather than
  dropped, and a chunk whose MAD is zero records `degenerate_noise_scale`.
...
- `exclusions` records insufficient baselines, sub-threshold candidates, shapes
  truncated by acquisition boundaries or gaps, and windows that carry no usable
  noise estimate. A MAD family has no threshold where its scale estimate
  degenerates, so a flat or quantization-locked window rejects its candidates as
  `degenerate_noise_scale` and a window with fewer than three samples rejects
  them as `insufficient_noise_samples`. Neither is clamped to a small floor,
  because a floored scale accepts every local maximum in the window.
...
`.interaction()`. The [cross-workflow population guide](population-workflow-adapters.md)
