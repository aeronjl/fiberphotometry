...
[multiscale long-duration workflow](multiscale-long-duration.md). It shares
...
See [irregular sampling](irregular-sampling.md) for the separate resampling
contract.
...
A ratio contrast is computed entirely on the log scale: the reported estimate is
the **geometric** mean of the paired per-animal ratios, and its interval is the
bootstrap percentile interval of the mean log ratio, back-transformed by
exponentiation. The estimand is recorded as
`geometric_mean_of_paired_state_ratios` and the interval scale as
`bootstrap_percentile_of_mean_log_ratio_back_transformed`, so a reader never has
to guess which mean was taken. The randomization test likewise flips paired log
ratios. A geometric mean is the correct centre for a multiplicative quantity: it
is invariant to inverting the contrast, whereas an arithmetic mean of ratios is
not, and the back-transformed interval stays strictly positive.
...
contract. See [population inference across core workflows](population-workflow-adapters.md).
The existing specialist function remains the route for ratio estimands and paired
sign-flip evidence.

Use the [population-curve workflow](population-curve-inference.md) when the
