...
[two-group × two-condition interaction](population-interactions.md), which
...
[spontaneous transients, state-band power, and multi-signal association](population-workflow-adapters.md)
without erasing their domain-specific exposure, window, or pair-count denominators.
The [population-curve workflow](population-curve-inference.md) carries complete
